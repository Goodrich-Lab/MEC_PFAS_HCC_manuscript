...
# g_mim <- read.graph(fs::path(dir_results, 
#                          "ComptoxAI",
...
plot(node_metadata$authority, node_metadata$hub_score)


# filter hub score > 0
temp <- node_metadata |> 
  tidylog::filter(hub_score > 0.1)
...
pfas_proteomics <- pfas_proteomics |> 
  dplyr::mutate(
    geneSymbol = case_when(EntrezGeneSymbol == "FGF8_3" ~ "FGF8", 
                           ...
                           EntrezGeneSymbol == "IL27|EBI3" ~ "EBI3",
                           EntrezGeneSymbol == "GDF11|MSTN" ~ "MSTN",
...
(cor_plot <- ggplot(ppw_rank, aes(x = strength, y = pm, label = geneSymbol)) +
