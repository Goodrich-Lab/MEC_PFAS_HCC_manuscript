# Regenerate an overall supplemental table for HEP-status-stratified analysis
# by combining:
#   Table S3. PFAS HCC association by HEP status_081026.xlsx  (continuous PFAS-HCC, by HEP status)
#   Table S3. Dose response_by_hep_081026.xlsx                (categorical/quartile dose-response, by HEP status)
#   Table S5. Heterogeneity test_081026.xlsx, type == "HEP"
#     (Cochran's Q test for heterogeneity between Viral and Non-Viral cases)
#
...
s3  <- readxl::read_xlsx(fs::path(dir_supp, "Table S3. PFAS HCC association by HEP status_081026.xlsx"))
s2  <- readxl::read_xlsx(fs::path(dir_supp, "Table S3. Dose response_by_hep_081026.xlsx"))
s5  <- readxl::read_xlsx(fs::path(dir_supp, "Table S5. Heterogeneity test_081026.xlsx"))

# Sample sizes are a fixed label (as with the "(n = ...)" labels in
# reorganize.R, not derivable from the files used here) provided directly by
# the study team.
groups <- list(
  list(key = "Viral Cases",     label = "Viral Cases\n(n = 128)"),
  list(key = "Non-Viral Cases", label = "Non-Viral Cases\n(n = 318)")
)

pfas_dose_order <- unique(s2$pfas_name)
pfas_no_dose    <- setdiff(s3$pfas_name, pfas_dose_order)

het <- s5 %>%
  dplyr::filter(type == "HEP") %>%
...
build_group_columns <- function(key) {

  or_col    <- paste0("Odds Ratio[95%CI]_", key)
  p_col     <- paste0("P-Value_", key)
  trend_col <- paste0("P trend_", key)
  qval_col  <- paste0("Q value_", key)
  ...
  cont_rows <- s3 %>%
    dplyr::filter(pfas_name %in% pfas_dose_order, group == key) %>%
  ...
  no_dose_block <- s3 %>%
    dplyr::filter(pfas_name %in% pfas_no_dose, group == key) %>%
...
group_columns <- lapply(groups, function(g) build_group_columns(g$key))
...
openxlsx::saveWorkbook(wb, fs::path(dir_modified, "Table S3. Dose response_by_hep_M.xlsx"), overwrite = TRUE)
